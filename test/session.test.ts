...
test("error - wrong password", (t) => {
  ...
  const salt = TestRoutines.generateRandomSalt(16);
  ...
  const serverSession = new SRPServerSession(TestRoutines);
  const B = serverSession.step1(testUsername, salt, verifier);

  const clientSession = new SRPClientSession(TestRoutines);
...
test("#SRPSessionGetters success (set values)", (t) => {
  ...
  t.equals(
    ...
    wordArrayToBigInt(
      hash(session.routines.parameters, bigIntegerToWordArray(session.S)),
    ),
  );
  t.end();
});
