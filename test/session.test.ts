...
test("#SRP6a_Nimbusds_Session success", (t) => {
  ...
  Array.from(Array(TEST_COUNT).keys()).forEach((i) => {
    ...
    const server = new SRPServerSession(TEST_ROUTINES).step1(
      testUsername,
      salt,
      verifier,
    );

    const client = new SRPClientSession(TEST_ROUTINES).step1(
      testUsername,
      testPassword,
    );
...
test("error - wrong password", (t) => {
  ...
  const salt = TEST_ROUTINES.generateRandomSalt(16);
  ...
  const clientSession = new SRPClientSession(TEST_ROUTINES).step1(
    testUsername,
    diffPassword,
  );
  const { A, M1 } = clientSession.step2(salt, server.B);
